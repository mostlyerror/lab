class WizardsController < ApplicationController
  ...
  def validate_step
    ...
    @user_wizard.user.attributes = uzer_wizard_params
    session[:user_attributes] = @user_wizard.user_attributes
  ...
  def create
    if @user_wizard.user.save
      ...
      redirect_to({action: Wizard::User::STEPS.first }, alert: 'There was a problem when creating the user.'
    end
  end
  ...
  def wizard_user_next_step(curren_step)
  ...
  def user_wizard_params
    params.require(:user_wizard).permit(:email, :first_name, :last_name, :address_1, :address_2, :zip_code, :city, :state, :country, :phone_number)
  end

  def class InvalidStep < StandardError; end
end
